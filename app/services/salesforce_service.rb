...
class SalesforceService
  ...
  def subscribe!
    ...
    Rails.logger.info("SalesforceService#subscribe! enter, email: #{follow_up.email}, campaign_name: #{follow_up.destination.service_name}, data: #{data.inspect}")

    return false unless (access_token = self.class.get_access_token)
    ...
    payload = [
      {
        keys: {
          subscriberkey: follow_up.email,
          campaign: follow_up.destination.service_name
        },
        values: {
          subscriberkey: follow_up.email,
          campaign: follow_up.destination.service_name,
    ...
    Rails.logger.info("SalesforceService#subscribe! payload: #{payload.to_json}\n\nheaders: #{headers.inspect}\n\nurl: #{url.inspect}\n\n")
    response = RestClient.post(url, payload.to_json, headers)
    Rails.logger.info("SalesforceService#subscribe! response: Code: #{response.code}\nHeaders: #{response.headers.inspect}\n\nBody: #{response.body}")
    ...
    Rails.logger.error("SalesforceService#subscribe! Failed to send campaign data to Salesforce: #{e.message}\n#{e.backtrace}")
    raise
  end
